...
describe UsersController do
  ...
  describe "#update" do
    ...
    context "params_handling" do
      ...
      # break this in the cases below
      skip "a user updating himself"
      skip "a user updating another user"
      skip "a superuser updating himself"
      skip "a superuser updating another user"
    ...
    describe "with institutions" do
      describe "setting :can_record" do
        ...
        context "an institutional admin" do
          ...
          context "cannot set to true if the institution doesn't have free slots" do
            ...
            it { should render_template('edit') }
            it { should render_with_layout('no_sidebar') }
            it { should set_flash.to(I18n.t('users.update.error')) }
            it {
      ...
      describe "setting :approved" do
        before(:each) { @institution = FactoryGirl.create(:institution) }
      ...
      describe "setting :disabled" do
        ...
        context "an institutional admin" do
          ...
          context "can set it for users of his institution" do
            before(:each) do
              @user = FactoryGirl.create(:user, disabled: false)
              @institution.add_member!(@user)
              put :update, :id => @user.to_param, :user => { disabled: true }
            end
            ...
            it { should redirect_to edit_user_path(@user) }
            it { should set_flash.to(I18n.t('user.updated')) }
            it { @user.reload.disabled.should be(true) }
          end
        end
      end
    end
  ...
  describe "#approve" do
    ...
    context "if #require_registration_approval is set in the current site" do
      ...
      context "but the user's approval fails" do
        ...
        it { should respond_with(:redirect) }
        it { should set_flash.to(I18n.t('users.approve.institution_full', :name => user.institution.name, :limit => user.institution.user_limit)) }
        it { should redirect_to(referer) }
        it("doesn't approve the user") { user.reload.approved?.should be(false) }
        it("doesn't confirm the user") { user.reload.confirmed?.should be(false) }
      end
  ...
  describe "#create"  do
    ...
    describe "as a institution admin" do
      ...
      describe "if institution has free slots and the slots for recording are full" do
        ...
        it { should set_flash.to(I18n.t('manage.users.create_without_record')) }
        it { should redirect_to manage_users_path }
        it { User.last.confirmed?.should be true }
        it { User.last.institution.should eql(institution) }
