...
describe BigbluebuttonRoom do
  ...
  describe "from initializers/bigbluebutton_rails" do
    ...
    describe "#server_considering_secret" do
      let(:default) { FactoryGirl.create(:bigbluebutton_server, secret: "default-secret") }
      before { BigbluebuttonServer.stub(:default).and_return(default) }
      ...
      context "if there's a current meeting" do
        ...
        context "not ended with a secret, uses its secret" do
          subject { room.server_considering_secret }
          it { subject.url.should eql(default.url) }
        ...
        context "not ended but without a secret, doesn't use it" do
          ...
          subject { room.server_considering_secret }
          it { subject.should eql(default) }
          it { subject.new_record?.should be(false) }
        end
      ...
      context "if there's no current meeting" do
        ...
        context "if the room has an institution, uses its server" do
          subject { room.server_considering_secret }
          it { subject.should eql(institution.server) }
          it { subject.new_record?.should be(false) }
        end

        context "if the room has no owner, uses the default server" do
        ...
        context "if the room's owner has no institution, uses the default server" do
        ...
        context "if the room has an institution that doesn't have a secret, uses the default server" do
